...
# ── project config (inline, T+7 only — the original 4-route single-window
#    working version. Multi-window version lives in scraper.py) ─────────────
import random
...
# T+7 only — this is the original single-window version.
BOOKING_WINDOWS = [7]
DAYS_AHEAD = 7                              # T+7 primary window
CABIN_CLASS = "Economy"
...
class AirIndiaScraper:
    ...
    def _fill_date(self, date_str: str):
        ...
        for 8 September 2026. The calendar opens on the current month so T+7
